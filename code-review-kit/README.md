...
2026-09-19／20 以真實 API 跑過 16 次、6 個真實標的（5 個歷史 commit + 1 個實際 PR），整套花費 **$0.21**：
...
| TypeScript + React 實際 PR（27 檔，**註解密度高**） | 未驗（沒走 `post_review.py`） | ⚠️ **16 筆 finding 只有 1 筆成立**——見 §4.8，這是這個工具最該避開的標的類型 |
...
16 次真實 API 呼叫、6 個真實標的。模型講的每一句話都另外驗證過——
...
* **`deepseek_review.py` 對真實 DeepSeek API 端到端跑通**（2026-09-19／20，16 次呼叫、
  6 個真實標的，總花費 $0.21）。其中 4 次是同一份 PR diff 的 2×2 對照
  （2 模型 × 2 版 rubric），結論見 §4.7／§4.8。這條在 2026-09-17 的版本裡還是「未驗證」——
