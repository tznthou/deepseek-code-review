...
2026-09-19／20 以真實 API 跑過 17 次、7 個真實標的（5 個歷史 commit + 1 個實際 PR + 本 kit 自己），整套花費 **$0.23**：
...
17 次真實 API 呼叫、7 個真實標的。模型講的每一句話都另外驗證過——
...
* **`deepseek_review.py` 對真實 DeepSeek API 端到端跑通**（2026-09-19／20，17 次呼叫、
  7 個真實標的，總花費 $0.23）。其中 4 次是同一份 PR diff 的 2×2 對照
  （2 模型 × 2 版 rubric），1 次是拿本 kit 審自己——**撈出兩個真 bug 並修掉**，
  結論見 §4.7／§4.8。這條在 2026-09-17 的版本裡還是「未驗證」——
