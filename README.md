...
├── tools/
│   ├── selftest.py                         # 不需網路/API key 的自測（18 組 100 項）
...
#### 2026-09-22 的五次 dogfood：7 筆 finding、**0 筆成立**

同一天三個 PR、連續五次 run（都是這個 kit 自己的改動）的實際數字：
...
| #22 | parser 修正 | 1 | **0** |
| #21 第二次 | 同上 + 文件 | 3 | **0** |
| #21 第三次 | 同上 + merge main（帶進 #22） | 3 | **0** |

其中三筆的失效模式各不相同，而且都值得記：
...
⚠️ **2026-09-23 更正**：這段原本寫「四次、4 筆」——#21 的第二、三次 run 被當成了同一次。
兩次各 3 筆、內容完全不同（上面第 1 筆出自第二次，第 2、3 筆出自第三次），
於是有 3 筆從沒被記錄。補驗後同樣不成立：

* 「`REVIEW_BLOCKED_TERMS` 可能沒有傳到 Python」（`major`、信心 0.8）——它自己的 evidence
  寫著「未顯示後續 `run` 區塊如何使用該變數」，而 Python 端讀 `os.environ` 的那行
  就在同一份 diff 的另一個檔裡（diff 19,392 字元、沒有截斷）。同一天的真實 CI 也
  實測到攔截生效（§8）。
* 「`blocked_terms_hits` 沒有處理 None」——`(text or "")` 已經處理了，又是報已經做了的事。
* 「環境變數裡的非 UTF-8 字元會拋例外」——讀進來一律是 `str`，非法位元組會變成
  surrogate；實測不拋例外，照樣比對得到。

⚠️ **這批數字不推翻前面的結論，也不強化它。** 三個 PR 的改動以文件、測試與
錯誤訊息為主，正是 §4.8 說的「低訊號標的」；樣本也太小。放在這裡是因為
**失效模式比成立率有資訊量**——尤其第 3 筆，它的斷言可以用同一天的實測直接證偽。

一個正面訊號：那批裡兩筆 `major`（第二、三次各一筆，指的是同一行）都因為「片段多重
命中（2 處），不猜」被定位層擋下、沒貼成 inline。那是 `locate.py` 按設計在運作
（見 §8「行號改由程式用程式碼片段比對算出」那條）。

至於 #21 第一次那個**解析失敗**——那不是模型的問題，是 `extract_json` 自己的
bug（把 finding 內容裡的 code fence 當成整份回應的外包裝），`v1.3.0` 修掉了。

#### `v1.3.0` 到 `v1.3.1`（#23–#28）：2 筆 finding、**1 筆成立**

接下來六個 PR、七次 run：

| PR | 內容 | finding | 逐筆驗證後成立 |
|---|---|---|---|
| #23 | 發版 `v1.3.0` | **`04` startup_failure**（§7 那個兩分鐘的破窗） | — |
| #24 | 文件 | 0 | — |
| #25 | 文件 | 0 | — |
| #26 | 文件 | 0 | — |
| #27 第一次 | ReDoS 修正 + selftest | 2 | **1** |
| #27 第二次 | 補回被吃掉的空格 | 0 | — |
| #28 | 發版 `v1.3.1` | 0 | — |

成立的那筆是 `tools/selftest.py` 的 `ROOT =pathlib.Path(...)`：編輯時替換字串結尾的空格
被吃掉，編輯工具照樣回報成功。只是一筆 `minor` 的排版問題，值得記是因為**我們原本預期
它是誤報**，查了才發現是真的。本 repo 的 CI 沒有跑 Python linter（`01` 只跑 shellcheck），
CI 裡只有它會抓到這一筆。

不成立的那筆說 selftest 的時間探針（0.5 秒門檻）「在 CI 環境中可能因系統負載而偶發超時」：
前提不存在——selftest 不在 CI 跑；而且修正後的實作 128K 字元只要 0.04 毫秒，
離門檻四個數量級。

⚠️ 同樣是小樣本、以文件為主的標的，不拿來算成立率。
...
* `python3 tools/selftest.py` **18 組 100 項斷言全通過**。
